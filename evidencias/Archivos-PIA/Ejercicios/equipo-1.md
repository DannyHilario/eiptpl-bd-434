...
**LibroMás** es una librería que organiza su catálogo por categorías: ficción, ciencia, historia y autoayuda, entre otras. Cada libro tiene un precio unitario que puede cambiar con el tiempo.

Actualmente, el encargado registra todas las ventas en una hoja de cálculo donde cada fila representa una venta. La hoja contiene la siguiente información en columnas:

> Categoría del libro, título del libro, autor, precio unitario, nombre completo del cliente, teléfono del cliente, correo electrónico del cliente, fecha de venta, cantidad.
...
- Cuando el precio de un libro cambia, es necesario actualizar manualmente cada fila histórica que lo contenga, lo que ha generado registros con precios contradictorios para el mismo título.
- No es posible saber qué libros forman parte del catálogo si ninguno ha sido vendido todavía.
...
Scripts SQL para crear y poblar la base de datos desde cero. Nómbralos con prefijo numérico que indique el orden de ejecución (por ejemplo: `1_create-database.sql`, `2_create-table-categoria.sql`, `3_create-table-libro.sql`, `4_create-table-cliente.sql`, `5_create-table-venta.sql`, `6_insert-categoria.sql`, `7_insert-libro.sql`, `8_insert-cliente.sql`, `9_insert-venta.sql`).
...
- ¿Qué libros pertenecen a la categoría de ciencia?
- ¿Cuáles son las 10 ventas más recientes?
- ¿Qué clientes no han realizado ninguna compra todavía?
- ¿Qué libros nunca han sido vendidos?
- ¿Qué ventas se realizaron en el primer trimestre del año?
- Muestra todas las ventas con el nombre del cliente, el título del libro y la categoría a la que pertenece.
